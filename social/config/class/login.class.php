<?php
  class login_class{
    ...
    //return "SELECT id FROM users WHERE username = ".$username." AND password = ".$password." LIMIT 1"; //Timothy Kile 12.30.2019 7:46 PM
      if (($username || $password) == "") {
        ...
        if ($query->rowCount() == 0) {
          ...
          if (password_verify($password, $pass)) {
            ...
            $id = $irow->id;
            $emailquery = $this->db->prepare("SELECT email FROM users WHERE username = :username AND password = :password LIMIT 1");
            $emailquery->execute(array(":username" => $username, ":password" => $pass));
            $emailrow = $emailquery->fetch(PDO::FETCH_OBJ);
